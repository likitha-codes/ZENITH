...
A less sensitive representation can be derived from the original information.

This allows the system to retain useful meaning without exposing the original value.
...
ZENITH does not rely on the model to decide whether the PAN can leave the system.
...
The external AI does not receive the blocked request.

The security event is recorded in the audit metadata.
...
Detects sensitive information in incoming requests.

Current supported entity types include:
...
Uses Featherless AI to understand the requested task and identify potentially required information.

necessityEngine.ts

Compares detected sensitive information against the information required for the task.

policyEngine.ts

Applies deterministic security policies to necessity results.

transformers.ts

Implements the available privacy actions:
...
Handles communication between ZENITH and Featherless AI.

outputGuard.ts

Inspects AI responses for sensitive information before returning them to the organization.

auditLogger.ts

Creates privacy-safe audit metadata.

firewall.routes.ts

Coordinates the complete firewall pipeline:
...
Visual representation of the ZENITH privacy firewall and its agents.

Agent Page

Explains the purpose of an individual security agent.

Chat Page

Provides the organizational interface for sending requests through ZENITH.
...
For production deployments, configure environment variables directly through the deployment platform.
...
When deploying the backend to Render, the Render service's outbound IP ranges must be allowed in MongoDB Atlas Network Access.

This ensures that the deployed backend can establish a connection to the MongoDB cluster.

📈 Privacy vs Utility

A privacy system should not simply remove as much information as possible.

If too much information is removed, the AI may no longer be able to perform the requested task effectively.
...
1. Least Necessary Data

The AI should receive the minimum information required to perform its task.

2. Context-Aware Privacy

Sensitivity depends not only on the data itself, but also on whether it is necessary for the current task.

3. Deterministic Enforcement

Security decisions should be predictable and enforceable through explicit rules.

4. Bidirectional Protection

Both AI input and AI output need privacy protection.

5. Privacy-Safe Observability

Organizations need auditability without creating another repository of raw sensitive information.
...
How much data can be removed?

while measuring:

Does the AI still perform the task effectively?
...
An organization may have access to:

100 pieces of information

but an AI model may need only:
...
ZENITH is developed as a two-person project.
